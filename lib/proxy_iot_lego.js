...
exports.newInstance = async function($, spec) {
    try {
        ...
        *
         ...
         *
         * @param {string} device A target device, identified by its
         ...
         * @alias callMethod
         */
        that.callMethod = function(device, methodName, args) {
            assert(DeviceTypes[device], 'Invalid device type');
            assert(typeof methodName === 'string');
            assert(Array.isArray(args));

            return $._.callMethod(device, methodName, args);
        };
